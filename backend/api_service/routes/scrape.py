...
async def make_scraper_request(scraper_url: str, exchange: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.post(f"{scraper_url}/api/v1/scrape", params=params, timeout=REQUEST_TIMEOUT_SECONDS)

            if response.status_code == 200:
                logger.info(f"Scan triggered successfully for {target_description}")
                return {"message": f"Scan triggered successfully for {target_description}", "status": "success"}
            else:
                ...
                return {"message": f"Scan request sent for {target_description}", "status": "success"}
    except Exception as e:
        logger.warning(f"Error communicating with scraper service: {str(e)}")
        # Just return success anyway - the scan is asynchronous
        return {"message": f"Scan initiated for {target_description}", "status": "success"}
