...
class StatsService:
    ...
    async def get_listing_stats(self, days: int = 30, exchange_code: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    async def _get_summary_stats(self, since: datetime, exchange_code: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            statuses = {}
            status_query = (
                select(StockListing.status, func.count(StockListing.id)).join(Exchange).where(StockListing.listing_date >= since.replace(tzinfo=None))
            )
            ...
            security_types = {}
            type_query = (
                select(StockListing.security_type, func.count(StockListing.id))
                .join(Exchange)
                .where(StockListing.listing_date >= since.replace(tzinfo=None))
            )
            ...
            return {"total": total_filtered, "total_all_time": total_all_time, "statuses": statuses, "security_types": security_types}
        except Exception as e:
            raise DatabaseQueryError(f"Failed to get summary statistics: {str(e)}") from e

    async def _get_exchange_stats(self, since: datetime, exchange_code: Optional[str] = None) -> list:
        """Get statistics by exchange."""
        try:
            # Query for exchange breakdown
            query = (
                select(Exchange.code, Exchange.name, func.count(StockListing.id).label("count"))
                .join(StockListing)
                .where(StockListing.listing_date >= since.replace(tzinfo=None))
            )
    ...
    async def _get_daily_counts(self, since: datetime, exchange_code: Optional[str] = None) -> list:
        ...
        try:
            ...
            query = (
                select(date_extract.label("date"), func.count(StockListing.id).label("count"))
                .join(Exchange)
                .where(StockListing.listing_date >= since.replace(tzinfo=None))
            )
            ...
            for date, count in result:
                daily_counts.append({"date": date.strftime("%Y-%m-%d") if hasattr(date, "strftime") else str(date), "count": count})
            ...
            while current_date <= end_date:
                ...
                filled_counts.append({"date": date_str, "count": date_dict.get(date_str, 0)})
